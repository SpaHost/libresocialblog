(function () {
	...
	var num_replies = '2,0,0,0,0,0'.split(',');
	...
	for (i = 0; i < disqus_nodes.length; i++) {
		...
		if (replies_count !== undefined && !isNaN(replies_count)) {
			if (replies_count > 1) {
				link_text += '<span class="badge badge-info">{num} Comentarios</span>'.replace('{num}', replies_count);
			} else if (replies_count) {
				link_text += '<span class="badge badge-info">1 Comentario</span>'.replace('{num}', replies_count);
			} else {
				link_text += '<span class="badge badge-info">0 Comentarios</span>'.replace('{num}', replies_count);
			}
		}
